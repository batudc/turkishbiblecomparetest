#!/usr/bin/env python3
"""
Agent 2: Translate short_def field → Tanım
Outputs: data/strongs/tr2_def_heb.json, tr2_def_grk.json
Format: {"H123": "translated definition", ...}
"""
import json, time
...
def process(lang_file: Path, out_file: Path, prefix: str):
    ...
    for e in source:
        ...
        if key in existing:
            ...
            continue
        text = e.get('short_def') or e.get('nas_def') or ''
...
def main():
    process(STRONGS / 'hebrew.json', STRONGS / 'tr2_def_heb.json', 'H')
    process(STRONGS / 'greek.json',  STRONGS / 'tr2_def_grk.json', 'G')
    print('\n=== Agent 2 complete ===')
